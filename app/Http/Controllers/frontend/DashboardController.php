    ...
    public function buyCredit(Request $request)
       ...
       public function studentTutor(Request $request)
       {
         $tutors = DB::table('tutor_assign')
                  ...
                  ->join('students','students.student_id','=','students.student_id')
                  ->where('tutor_assign.user_id','=',auth()->user()->id)->where('students.user_id','=',auth()->user()->id)->orderby('users.first_name')->orderby('students.student_name')->paginate(10);
                  // dd($tutors);
       ...
       public function addEditSession(Request $request){
         $session_id = 0;
           ...
           if($request->isMethod('post')){
             ...
             if ($date2 > $date1) {
               // dd($date1,$date2);
               $sMsg = 'YUnable to schedule a session in the past. Please correct the session date or time and try again.';
